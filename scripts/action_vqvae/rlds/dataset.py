...
def make_action_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        ...
        # 只保留需要的数据。保留绝对动作掩码，供 chunk_act 在轨迹尾部
        # 对相对 EEF 维度补零、对绝对夹爪维度重复最后状态。
        result = {
        ...
        if absolute_action_mask is not None:
            if len(absolute_action_mask) != result["action"].shape[-1]:
                raise ValueError(
                    ...
                    f"does not match action dimension ({result['action'].shape[-1]})."
                )
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    if sampling_stride > 1:
        dataset = dataset.traj_map(
            partial(traj_transforms.stride, sampling_stride=sampling_stride),
            num_parallel_calls,
        )
...
def make_interleaved_dataset(
    ...
    *,
    ...
    # Get indices of primary datasets.
    primary_dataset_indices = np.flatnonzero(np.asarray(sample_weights, dtype=np.float64) == 1.0)

    # Some custom mixtures use the same non-1 weight for every dataset.
...
def make_interleaved_action_dataset(
    ...
    *,
    ...
    Args:
        ...
        traj_read_threads: total number of parallel read workers for trajectory transforms, distributed across
            datasets according to their sampling weights. If None, defaults to AUTOTUNE for every dataset.
        apply_shuffle: whether to apply this dataset's final shuffle/cache stage. Hybrid storage disables it so the
            combined tar and TFDS action stream can use one shared shuffle buffer.
    """
    # Default to uniform sampling (if `sample_weights` is not specified)
    if not sample_weights:
        sample_weights = [1.0] * len(dataset_kwargs_list)
