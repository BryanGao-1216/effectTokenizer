...
def _decode_kuka_state(value: tf.Tensor, width: int) -> tf.Tensor:
    """Decode legacy ZLIB strings or pass through materialized float states."""
...
def kuka_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Original RLDS releases store these values as ZLIB-compressed strings,
    # while the jxu124/OpenX tar release materializes them as float arrays.
...
def nyu_franka_play_dataset_transform(
    ...
    # TFDS 中 depth 已经解码为数值；tar 中可能仍是编码后的字符串。
    # Action-only 训练不需要解码深度图。
    ...
    if depth_additional.dtype != tf.string:
        trajectory["observation"]["depth_additional_view"] = tf.cast(depth_additional[..., 0], tf.float32)

    trajectory["observation"]["eef_state"] = (trajectory["observation"]["state"][:, -6:])

    trajectory["action"] = tf.concat(
        (
            trajectory["action"][:, -8:-2], tf.clip_by_value(trajectory["action"][:, -2:-1], 0, 1),
        ),
        axis=-1,
    )
