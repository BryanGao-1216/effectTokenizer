...
def stride(traj: Dict, sampling_stride: int) -> Dict:
    """Keep trajectory windows whose start index is a multiple of ``sampling_stride``."""
    if sampling_stride <= 0:
        raise ValueError(f"sampling_stride must be positive, got {sampling_stride}")
    if sampling_stride == 1:
        return traj
    traj_len = tf.shape(traj["action"])[0]
    indices = tf.range(0, traj_len, delta=sampling_stride)
